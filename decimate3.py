...
class Decimater(obja.Model):
    ...
    def getError(self, Qs, pair):
        ...
        try:
            ...
            # En cas d'erreur lors de l'inversion
            # Calculer le point sur la droite v1 v2 qui minimise l'erreur
            v1 = np.array(self.vertices[a])
            v2 = np.array(self.vertices[b])
            v_on_line = self.minimize_error_on_line(Q, v1, v2)

            if v_on_line is not None:
                return v_on_line

            # Si aucun point sur la droite ne minimise l'erreur,
            # choisir le point qui minimise l'erreur entre v1, v2 et le milieu du segment
            v_midpoint = (v1 + v2) / 2
            e_v1 = (v1.T @ Q @ v1)[0, 0]
            e_v2 = (v2.T @ Q @ v2)[0, 0]
            e_midpoint = (v_midpoint.T @ Q @ v_midpoint)[0, 0]

            if e_v1 <= e_v2 and e_v1 <= e_midpoint:
                print("e_v1")
                return v1, e_v1
            elif e_v2 <= e_midpoint:
                print("e_v2")
                return v2, e_v2
            else:
                print("e_midpoint")
                return v_midpoint, e_midpoint

        e = (v.T @ Q @ v)[0, 0]

        return v, e

    def minimize_error_on_line(self, Q, v1, v2):
        print("minimize_error_on_line")
        ...
        for t in np.linspace(0, 1, 100):  # Divisez l'intervalle [0, 1] en 100 points
            v_test = v1 + t * direction
